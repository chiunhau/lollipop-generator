...
const sketch = (p, getProps) => {
  ...
  p.draw = () => {
    ...
    render(
      ...
      randomRotateMagicNumber,
      randomSizeMagicNumber,
      rotationOffsets,
      p.frameCount,
    )
  }
  ...
  function render(
    ...
    const size =
      Math.sin(t / randomSizeMagicNumber - Math.PI / 2) *
        getProps().size +
      getProps().size

    p.push()
    p.rotate(Math.sin((t / m) * rotationOffsets[0]))
    ...
    p.push()
    // rotate(QUARTER_PI * 1)
    p.rotate(Math.sin((t / m) * rotationOffsets[1]))
    ...
    p.push()
    // rotate(QUARTER_PI * 2)
    p.rotate(Math.sin((t / m) * rotationOffsets[2]))
    ...
    p.push()
    // rotate(QUARTER_PI * 3)
    // rotate(Math.sin((t / m) * rotationOffsets[3]))
